...
!=========================================================================
! read the crystal structure in POSCAR file with atomic positions in
...
! local variables  
  ...
  character(len=132) type_str, frame
!------------------------------------------------------------------------- 
  !
  ...
  else if(trim(adjustl(frame)) == 'C' .or. trim(adjustl(frame)) == 'c' .or. trim(adjustl(frame)) == 'Cartesian') then
    do i = 1, natom_conv
      read(10,*) atom_conv_cart(:,i)
      atom_conv_crys(:,i) = matmul(mat_inv(a_conv),atom_conv_cart(:,i))
    end do
  else
    write (stdout,*) 'Wrong frame option line in POSCAR'
    write (stdout,*) '"D" or "d" or "Direct" for crystal frame while "C" or "c" or "Cartesian" for Cartesian frame'
    stop
  end if
  close(10)
  !
