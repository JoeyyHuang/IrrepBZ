...
!=========================================================================
  ...
  use constants, only : tpi, tol, tol3
  implicit none
!-------------------------------------------------------------------------
! arguments
...
!-------------------------------------------------------------------------
  ...
  !
  if(abs(angle-tpi/2.d0) < tol) then     ! pi angle
    if(abs(rot(1,1)+1.d0) > tol) then
      n(1) = sqrt(abs(rot(1,1)+1.d0)/2.d0)
      n(2) = rot(1,2)/(2.d0*n(1))
      n(3) = rot(1,3)/(2.d0*n(1))
    else
      n(1) = 0.d0
      if(abs(rot(2,2)+1.d0) > tol) then
...
100 continue
  ...
  diff = 2*(axis/2) - axis
  if(all(abs(dble(diff)) < tol3)) then
    axis = axis/2
    return
  end if
  diff = 3*(axis/3) - axis
