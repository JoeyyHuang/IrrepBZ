...
!=========================================================================
! find from the chosen primitive basis vector the holohedry_point_group of lattice
!=========================================================================
  use variables, only : a_prim, holo_ele, holo_ord, crys_sys
  use mat_trans, only : mat_inv, mat_det
  use constants, only : err_tol, err_tol33
  implicit none
!-------------------------------------------------------------------------
! local variables
...
! lengths of basis vectors
  ...
  do i = 1, nvec
    do j = 1, nvec
      do k = 1, nvec
        ...
        if(all(abs(matmul(transpose(vec_mat),vec_mat)-matmul(transpose(a_prim),a_prim)) < err_tol33)) then
  ...
  do i = 1, holo_ord
    ! only need consider proper rotations
    if(mat_det(holo_ele(:,:,i)) > 0.d0) then
      trace = holo_ele(1,1,i) + holo_ele(2,2,i) + holo_ele(3,3,i)
      if(abs(trace+1.d0) < err_tol) rot2 = rot2 + 1
      if(abs(trace) < err_tol)      rot3 = rot3 + 1
      if(abs(trace-1.d0) < err_tol) rot4 = rot4 + 1
      if(abs(trace-2.d0) < err_tol) rot6 = rot6 + 1
    end if
  end do
  !
  ! do not disrupt the order
  if(rot2 == 1) crys_sys = 'Mono'   ! single 2-fold rotation
  if(rot2 == 3) crys_sys = 'Orth'   ! three 2-fold rotation
  if(rot4 == 2) crys_sys = 'Tetr'   ! single 4-fold rotation
  if(rot3 == 2) crys_sys = 'Trig'   ! single 3-fold rotation
  if(rot6 == 2) crys_sys = 'Hexa'   ! single 6-fold rotation
  if(rot3 == 8) crys_sys = 'Cubi'   ! four 3-fold rotation
  !
  return
