...
!=========================================================================
! find the isogonal_point_group based on the space group matrices
!=========================================================================
  use mat_trans, only : mat_det
  use constants, only : err_tol
  implicit none
!-------------------------------------------------------------------------
! arguments
...
!-------------------------------------------------------------------------
  ...
  do i = 1, spa_ord
    ...
    if(mat_det(spa_ele(:,:,i)) > 0.d0) then
      do j = -3, 3
        if(abs(trace+dble(j)) < err_tol) npro_list(4-j) = npro_list(4-j) + 1
      end do
    else if(mat_det(spa_ele(:,:,i)) < 0.d0) then
      do k = -3, 3
        if(abs(trace+dble(k)) < err_tol) nimp_list(4-k) = nimp_list(4-k) + 1
      end do
    end if
  end do
  !
